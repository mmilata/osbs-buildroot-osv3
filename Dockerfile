...
RUN pip install git+https://github.com/goldmann/docker-scripts && \
    cd /opt/ && git clone -b scratch-builds https://github.com/DBuildService/atomic-reactor.git && cd atomic-reactor && python setup.py install && \
    cd /opt/ && git clone -b scratch-builds https://github.com/DBuildService/osbs.git && cd osbs && python setup.py install
